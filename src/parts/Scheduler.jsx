...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, Grid, Paper, TextField, Typography, IconButton } from '@mui/material';
...
const[dataFetched, setDataFetched] = React.useState(false)
    ...
    return (
      ...
      <Button onClick={() => {setShowCalendar(false)}}>Termine</Button>
    </Box>
    <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        ...
        >
          ...
          <Paper sx={{ height: "100%", width: "86vw", overflow: "auto", backgroundColor: "#333e" }}>
            ...
            <Button onClick={() => setShowCalendar(true)} color='secondary'>Zurück</Button>
            <Typography variant="h6" color={"white"} fontSize={25} sx={{marginRight: 2}}>{value.format("ddd, MMM D")}</Typography>
        ...
        />
            ...
            {meetings
              ...
              >
                ...
                <Typography color="white" variant="h6" fontWeight="fontWeightBold" fontSize={20}>{meeting.title}</Typography>

                    <Typography color="white" variant="subtitle1">{meeting.start.format('HH:mm')} - {meeting.end.format('HH:mm')}</Typography>
                  </Box>
                  <Box sx={{ height: '1px', backgroundColor: 'black', margin: '5px' }} />
                <Typography color="white" variant="body1">{meeting.description}</Typography>
              </Paper>
            ))}
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="de">
          ...
          sx={{
            ...
            return (
              <Box sx={{ position: 'relative' }}>
                ...
                {hasMeeting && <span style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '4px', height: '4px', borderRadius: '50%', backgroundColor: 'red' }} />}
              </Box>
            );
          }}
        />
        {value && (
          <Paper sx={{ height: "100%", width: "66%", overflow: "auto", backgroundColor: "#333e" }}>
            <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0'}}>
            <Typography  color={"white"} sx={{fontSize: 30, marginLeft: 4}}>{value.format("ddd, MMM D")}</Typography>
            <IconButton aria-label="delete" color="secondary" onClick={handleClickOpen} sx={{outlineColor: "#333e"}} disableRipple disableFocusRipple>
              <AddIcon/>
            </IconButton>
          </Box>
            <Dialog open={open} onClose={handleClose} sx={{borderRadius: 40, '& .MuiDialog-paper': {
              borderRadius: 15,
...
/>
        ...
        />
              ...
              <DialogActions>
                <Button onClick={handleClose} color="secondary" sx={{marginBottom: 1}} disableRipple disableFocusRipple>
                  Zurück
                </Button>
            ...
            {meetings
              ...
              >
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography color="white" variant="h6" fontWeight="fontWeightBold" fontSize={21}>{meeting.title}</Typography>
